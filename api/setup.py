...
def path_exists_but_not_file():
    raise Exception('The MMDB_PATH environment variable is set, but the file does not exist.')
...
def download_mmdb_file():
    try:
        ...
        raise Exception(f'An error occurred while downloading the MaxMind database file: {e}. Please verify that the LICENSE_KEY environment variable is correct, or specify a MMDB_PATH.')
    ...
    if len(mmdb_files) == 0:
        raise Exception('The MaxMind database file was not found in the downloaded tar.gz file. Please verify that the LICENSE_KEY environment variable is correct, or specify a MMDB_PATH.')
